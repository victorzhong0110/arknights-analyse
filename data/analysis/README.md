...
4. **元素爆条**：`ep_damage_ratio`(神经)/`burn.atk_scale`(灼燃)/`element_damage_scale` 积累 →
   爆条时间 `1000/元素DPS`（BOSS 阈值 2000 翻倍）；爆条触发伤害（神经6000元素伤害等）按
   `积累+10秒冷却` 周期均摊；爆条期间额外元素伤害(`element_atk_scale`/`extra_ep_damage_scale`)
   按占空比计入；损伤类型按描述识别（神经/灼燃/侵蚀/凋亡/狂躁）
